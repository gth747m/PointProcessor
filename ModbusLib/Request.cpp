...
namespace ModbusLib
{
    ...
    void build_read_request(int slaveAddress, int startAddress, int numRegisters, std::vector<char>* msg)
    {
        union {
        ...
        msg->push_back(static_cast<char>(slaveAddress));
        // Set the function we want from the remote device
        msg->push_back(static_cast<char>(FunctionCode::READ_HOLDING_REGISTER));
    ...
    void build_write_request(int slaveAddress, int startAddress, std::vector<int>* values, std::vector<char>* msg)
    {
        union {
            char swap_int_bytes[4];
            int swap_int;
        ...
        msg->push_back(static_cast<char>(slaveAddress));
        // Set the function we want from the remote device
        msg->push_back(static_cast<char>(FunctionCode::WRITE_MULTIPLE_REGISTERS));
        // Set the starting address 
        swap_int = (startAddress - static_cast<int>(Parameter::MODBUS_REGISTER_OFFSET));
        // Swap endianess: Windows/Linux = little endian, Modbus = big endian
        msg->push_back(swap_int_bytes[3]);
        ...
        swap_int = values->size() * 2;
        msg->push_back(swap_int_bytes[3]);
        msg->push_back(swap_int_bytes[2]);
        // Set the number of bytes we're sending
        msg->push_back(static_cast<char>(values->size() * 4));
        // Data loop
    ...
    /// <param name="values">Array of values to set the registers to</param>
    /// <param name="msg">Modbus message buffer requesting a write of registers</param>
    void build_write_request(int slaveAddress, int startAddress, std::vector<float>* values, std::vector<char>* msg)
    {
        union {
            char swap_int_bytes[4];
            int swap_int;
