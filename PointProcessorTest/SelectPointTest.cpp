...
TEST(SelectPoint, SelectUInt32)
{
    ...
    point.calculate();
    ASSERT_EQ(point.get_value(), 2);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(2);
    point.calculate();
    ASSERT_EQ(point.get_value(), 4);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(3);
    point.calculate();
    ASSERT_EQ(point.get_value(), 6);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(0);
    point.calculate();
...
TEST(SelectPoint, SelectInt32)
{
    ...
    auto point = SelectPoint<int32_t, int32_t, int32_t>();
    point.add_input(&inpA);
    point.add_input(&inpB);
    point.add_input(&inpC);
    point.set_index_point(&indx);
...
TEST(SelectPoint, SelectInt64)
{
    ...
    auto point = SelectPoint<int64_t, int64_t, uint64_t>();
    point.add_input(&inpA);
    point.add_input(&inpB);
    point.add_input(&inpC);
    point.set_index_point(&indx);
...
TEST(SelectPoint, SelectFloat)
{
    ...
    indx.set_quality(Quality::GOOD);
    point.calculate();
    ASSERT_EQ(point.get_value(), 2.5f);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(2);
    point.calculate();
    ASSERT_EQ(point.get_value(), 4.2f);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(3);
    point.calculate();
    ASSERT_EQ(point.get_value(), 6.7f);
    ASSERT_EQ(point.get_quality(), Quality::GOOD);
    indx.set_value(0);
    point.calculate();
    ASSERT_EQ(point.get_quality(), Quality::NOT_CALCULABLE);
...
TEST(SelectPoint, SelectDouble)
{
    ...
    auto point = SelectPoint<float, double, int64_t>();
    point.add_input(&inpA);
    point.add_input(&inpB);
    point.add_input(&inpC);
    point.set_index_point(&indx);
